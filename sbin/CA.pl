...
# optimised for use with LoxBerry
...
my $CA_CONFIG = "-config $ENV{'LBHOMEDIR'}/config/system/ca.cnf" || "";
my $DAYS = "-days 365";
...
my $CA = "$openssl ca $CA_CONFIG -passin env:OPASS";
my $VERIFY = "$openssl verify";
my $X509 = "$openssl x509";
my $PKCS12 = "$openssl pkcs12";

# default openssl.cnf file has setup as per the following
my $CATOP = "$ENV{'LBHOMEDIR'}/data/system/LoxBerryCA";
my $CAKEY = "cakey.pem";
my $CAREQ = "careq.pem";
...
if ($WHAT eq '-newcert' ) {
    ...
    $RET = run("$CA -batch -policy policy_anything -out $NEWCERT $EXTRA{ca} -infiles $NEWREQ");
    $RET = run("$openssl rsa -in $NEWKEY -out $NEWKEYWP -passin env:OPASS");
