...
LOGINF "Installing Perl library libemail-mime-perl and libemail-sender-perl ...";

$output = qx { DEBIAN_FRONTEND=noninteractive /usr/bin/apt-get --no-install-recommends -q -y --fix-broken --reinstall install libemail-mime-perl libemail-sender-perl };
$exitcode  = $? >> 8;

if ($exitcode != 0) {
	LOGERR "Error installing libemail-mime-perl and/or libemail-sender-perl - Error $exitcode";
	LOGDEB $output;
	$errors++;
} else {
	LOGOK "libemail-mime-perl and libemail-sender-perl packages successfully installed";
}
