...
# Script version V3.0.0.4-OPTIMIZED
#
# Performance-Optimierungen gegenueber dem Original:
#
# FIX 1 (HOCH): Early-Filtering in received() - DoNotForward und Regex-Filter
#        werden VOR der teuren JSON-Expansion geprueft
# FIX 2 (HOCH): HTTP Connection-Pooling - Ein persistenter LWP::UserAgent
#        mit Keep-Alive statt neues Objekt pro Request
# FIX 3 (HOCH): get_miniservers() Cache - Miniserver-Config nur bei
#        Config-Reload lesen statt bei jedem HTTP-Call
# FIX 4 (MITTEL): Vorkompilierte Regexes - Subscription-Matching und
#        Filter-Expressions werden bei Config-Reload kompiliert
# FIX 5 (MITTEL): Eigene Gateway-Topics werden frueh gefiltert und
#        nicht an den Miniserver weitergeleitet (stoppt 404-Lawine)
# FIX 6 (NIEDRIG): Hash::Flatten wird einmal instanziiert statt pro Nachricht
# FIX 7 (NIEDRIG): JSON::XS wird bevorzugt statt JSON::PP
#
# Alle Aenderungen sind rueckwaertskompatibel - gleiche Config, gleiches
# Verhalten, gleiche Webinterface-Daten.
...
BEGIN {
	...
	if ($@) {
		# Fallback auf JSON::PP (bereits durch LoxBerry::JSON geladen)
		require JSON;
...
sub mshttp_call_fast {
	...
	my $url = $ms_cache{$msnr}{FullURI} . $command;
...
sub received
{
	...
	# Diese Topics (pollms, pollcpucurpct, etc.) werden vom PID-Regler
	# gepublished und landen wieder bei uns. Ohne diesen Filter durchlaufen
	# sie die komplette Pipeline und erzeugen 404s am Miniserver.
	...
	# der JSON-Expansion pruefen. Bei JSON-Topics muss die Filterung
	# nach der Expansion pro expandiertem Key passieren (wie bisher),
	# aber wir koennen zumindest den raw topic check vorab machen.
	...
	foreach my $sendtopic (keys %sendhash) {
		...
		# Run Subscription Filter Expressions (mit vorkompilierten Regexes)
		my $regexcounter = 0;
		my $regexmatch = 0;
	...
	# FIX 4: Nutze vorkompilierte Subscription-Regexes
	...
	if( is_enabled($cfg->{Main}{use_http}) ) {
		...
		if( $miniservers{$cfg->{Main}{msno}} ) {
			foreach ( @toMS ) {
				my $httpresp;
				# FIX 2: Nutze schnellen HTTP-Versand mit Connection-Pooling
